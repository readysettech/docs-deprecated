...
Check that it can be found using the retry-join feature.Create an IAM Managed Policy which has the ec2:DescribeInstances permission to find instances for retry-join. Copy this IAM Managed Policy ARN for use in Part 1.
...
Fill in the fields that are blank following the instructions in the gray text:
Fill in the IDs of the subnets referenced in .
To find the CIDR, navigate to the VPC console, find the row corresponding to your selected VPCID, and copy the CIDR.
