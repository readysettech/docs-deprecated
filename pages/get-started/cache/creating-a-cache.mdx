...
# Creating a Cache 
​
Once you've [identified queries](/get-started/cache/profiling-queries) that can benefit from caching with ReadySet, use ReadySet's custom SQL commands to check if the queries are supported and then to cache supported queries in ReadySet.
​
...
​
```sh copy 
CREATE CACHE [ALWAYS] [<name>] FROM <query string or query ID>;
```
...
--------------------+-----------------------------------------------+--------------------+-------
 q_bb6bd342f17bd8cd | SELECT                                       +| yes                |     1
                    |   *                                          +|                    |
                    | FROM                                         +|                    |
                    |   "employees"."employee"                     +|                    |
                    | WHERE                                        +|                    |
