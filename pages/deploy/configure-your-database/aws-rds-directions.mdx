...
# Configuring RDS via the AWS console to work with ReadySet

## Introduction 

If your database is an AWS RDS or Aurora instance, you'll need to make a few configuration changes
via the AWS console to enable ReadySet to properly snapshot your database and cache selected queries. 

There are three aspects that need to be true in order for ReadySet to properly work: 

- ReadySet must be able to communicate with your database
- Logical replication must be turned on in your database
- ReadySet requires superuser permissions

These changes can be performed in any order.

<Tabs items={['RDS/Postgres', 'RDS/Aurora']}>
<Tab>
## RDS/PostgresSQL
