...
<Callout> The easiest way to determine whether your queries are supported is to run them against Readyset and check the output of [`SHOW PROXIED QUERIES`](/get-started/cache#0-creating-a-cache). </Callout> 
## Query Structure
...
<Callout>Support for additional operators is always expanding! Reach out to us on [Discord](https://discord.gg/readyset), or [open an issue](https://github.com/readysettech/readyset/issues/new/choose) in our GitHub repository if you need an unsupported feature and we'll look into it.</Callout>
