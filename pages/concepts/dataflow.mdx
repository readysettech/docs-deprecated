...
ReadySet applies SQL operations over this data change stream to compute the results of the SQL queries you want to cache. 
...
the base tables that needs to be applied to compute the correct query result) and instantiates a long-lived **data flow graph** that 
...
```CREATE TABLE sotes (user int, story_id int);``` 
