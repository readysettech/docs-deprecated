...
maintain cached state in real time. In this model, both reader and intermediate nodes of the graph store result sets. 
...
Without partial materialization, ReadySet would have to compute the results for this query for ever possible 
input parameter (i.e. for every possible story). This would be pretty unfortunate, because caching the info 
for all stories is not necessary to improve the performance of most applications. In the case of HackerNews, 
...
traverses up the dataflow graph requesting its paren nodes for the state required to be able to rederive the missing result set. 
