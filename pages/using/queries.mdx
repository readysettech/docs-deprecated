...
- `JOIN` (see “Joins”)
- `WHERE`, with a single condition expression, which must be a supported expression (see “Expressions”) 
- `GROUP BY`, with a list of **column references**
ReadySet does not support expressions or field positions in the `GROUP BY` clause 

- `ORDER BY`, with a list of expressions, all of which must be supported expressions (see “Expressions”), and an optional ASC or DESC specifier 
- `LIMIT`, with a single **literal** row count

There are still specific top-level clauses and other query conditions that ReadySet does not support. Notably, ReadySet does not support any of the following (this is not an exhaustive list): 
- `HAVING`
- `OFFSET` 
...
The intricacies of whether particular queries are supported by ReadySet depend on multiple variables and you do not have to do work to figure out if your particular queries are supported by ReadySet. Instead, simply refer to the ReadySet dashboard, which will display information about whether individual queries are supported by ReadySet for you.
...
- Operators
    ...
    - Unary `NOT` 
    - `BETWEEN`
- `IN` and `NOT IN` with a list of expressions (see “Parameters”, however) 
- `CAST`
- A limited form of `CASE` (with only one `THEN` branch and an optional `ELSE` branch)
- Functions
    - `convert_tz`
    - `dayofweek` 
    - `ifnull` 
...
### Unsupported
ReadySet notably does not support any of the following components of the SQL expression language (this is not an exhaustive list): 
- Literals 
...
- `EXISTS` with a subquery (experimentally supported incl with correlation, but behind a flag) 
...
- Normally, parameters can only appear in the `WHERE` clause of the outermost `SELECT` statement in a query (e.g. not in any subqueries).
    - In addition, parameters are only supported in the `WHERE` clause of a query if, when expressed in conjunctive normal form, all conjunctive subexpressions of the expression in the `WHERE` clause either contain no parameters, or can be expressed as a single equality comparison between a column and a parameter, **or** are an `IN` expression where the right-hand side consists of a list of **only** parameters (ReadySet does not support mixing parameters and other types of expressions on the right-hand side of an `IN` expression).
...
- Parameters can also appear as the value of the `LIMIT` or `OFFSET` clause of a query, but only if ReadySet is able to statically determine that that query will always return either 0 or 1 row (specifically, this can happen if the `WHERE` clause of the query consists only of a filter on the primary key of a table, or on a column with a `UNIQUE` index).

### Limitations of `IN`
When the IN clause is used with parameters, queries may not contain the following elements: 
- Any aggregate function (`COUNT`, `SUM`,  `MAX`, `MIN`, `AVG`, or `GROUP_CONCAT`) 
- `ORDER BY` or `LIMIT` 
This limitation does not apply when the right-hand side of the `IN` clause does not contain any query parameters.  
